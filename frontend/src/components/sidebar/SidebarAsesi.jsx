...
const SidebarAsesi = ({ isOpen, setIsOpen }) => {
  ...
  // Daftar menu sidebar (hapus Konfirmasi Pembayaran)
  const menus = useMemo(
    () => [
  ...
  const isActive = (path) => {
    return path === '/asesi' ? location.pathname === '/asesi' : location.pathname.startsWith(path);
  };
  ...
  return (
    <>
      ...
      <div
        ...
        className={`fixed left-0 top-0 h-screen bg-[#071E3D] text-white flex flex-col z-[70] border-r border-white/10 transition-all duration-300 ${
