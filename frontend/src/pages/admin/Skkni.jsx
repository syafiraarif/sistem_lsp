...
import {
  Search, Plus, Eye, Edit2, Trash2, X, Save, FileText, Loader2, Filter, Sparkles, ClipboardList, Layers, BookOpen, Upload, FileSearch
} from "lucide-react";
...
const Skkni = () => {
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      console.error(error);
      notifikasi.gagal("Gagal", error.response?.data?.message || "Terjadi kesalahan saat menyimpan data");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (id) => {
    const confirmResult = await notifikasi.konfirmasi(
      "Konfirmasi Hapus",
      "Yakin ingin menghapus data Standar ini? Data yang terkait mungkin akan ikut terpengaruh!",
      "Ya, Hapus Data!",
      "Batal",
      "warning",
      "danger"
          ...
          <div className="relative z-10 grid grid-cols-1 gap-6 p-6 lg:p-8 xl:grid-cols-[1.15fr_0.85fr]">
            <div className="flex flex-col justify-center">
              <div className="mb-5 inline-flex w-fit items-center gap-2 rounded-full border border-orange-100 bg-orange-50 px-4 py-2">
                ...
                <span className="text-[10px] font-black uppercase tracking-widest text-orange-500">Data Standar / SKKNI</span>
              </div>
              ...
              <button type="button" onClick={openModal} className="mt-7 inline-flex w-fit items-center gap-2 rounded-2xl bg-orange-500 px-6 py-3 text-xs font-black uppercase tracking-widest text-white shadow-lg shadow-orange-500/20 transition-all hover:bg-[#071E3D]">
                <Plus size={16} />
                Tambah SKKNI
              </button>
            </div>
            ...
            <div className="relative overflow-hidden rounded-[32px] bg-[#071E3D] p-6 text-white shadow-2xl shadow-[#071E3D]/15">
              ...
              <div className="relative z-10">
                ...
                <p className="mb-2 text-[10px] font-black uppercase tracking-widest text-white/50">Total Standar</p>
                <h2 className="mb-4 text-5xl font-black leading-none">{dataList.length}</h2>
                <p className="text-sm font-medium leading-relaxed text-white/60">Ringkasan data standar yang tersedia dalam sistem.</p>
        ...
        <section className="overflow-hidden rounded-[32px] border border-slate-100 bg-white shadow-sm">
          <div className="border-b border-slate-100 p-6">
            <div className="mb-4 inline-flex items-center gap-2 rounded-full border border-orange-100 bg-orange-50 px-4 py-2">
              ...
              <span className="text-[10px] font-black uppercase tracking-widest text-orange-500">Daftar Standar</span>
            </div>

            <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
              <div>
                <h2 className="text-2xl font-black text-[#071E3D]">Data Standar / SKKNI</h2>
                <p className="mt-2 text-sm font-medium text-slate-400">Cari berdasarkan nomor atau judul standar, lalu filter berdasarkan jenis.</p>
              </div>
                ...
                {loading ? (
                  <tr>
                    <td colSpan="6" className="py-16 text-center">
                      ...
                      <p className="text-sm font-black uppercase tracking-widest text-[#071E3D]">Memuat Data Standar</p>
                    </td>
                  </tr>
                  ...
                  filteredData.map((item, index) => (
                    <tr key={item.id_skkni} className="border-b border-slate-100 transition-all last:border-0 hover:bg-orange-50/30">
                      ...
                      <td className="px-5 py-4">
                        <span className="font-mono text-sm font-black text-orange-500">{item.no_skkni}</span>
                      </td>

                      <td className="px-5 py-4">
                        <div className="max-w-[430px] font-black text-[#071E3D]">{item.judul_skkni}</div>
                      </td>

                      <td className="px-5 py-4 text-center">
                        <span className="inline-flex rounded-full border border-slate-100 bg-slate-50 px-3 py-1 text-[10px] font-black uppercase tracking-widest text-[#071E3D]">{item.jenis_standar}</span>
                      </td>
                      ...
                      <td className="px-5 py-4 text-center">
                        {item.dokumen ? (
                          <a
                            ...
                            target="_blank"
                            rel="noreferrer"
                            className="inline-flex items-center gap-2 rounded-full bg-[#071E3D]/10 px-4 py-2 text-[11px] font-black uppercase tracking-widest text-[#071E3D] transition-all hover:bg-orange-500 hover:text-white"
                          >
                            <FileText size={13} />
                            Lihat File
                          </a>
                      ...
                      <td className="px-5 py-4 text-center">
                        <div className="flex items-center justify-center gap-2">
                          <IconButton onClick={() => openDetailModal(item)} title="Lihat Detail" className="bg-[#071E3D]/10 text-[#071E3D] hover:bg-[#071E3D] hover:text-white">
                            <Eye size={16} />
                          </IconButton>

                          <IconButton onClick={() => handleEdit(item)} title="Edit Data" className="bg-orange-50 text-orange-500 hover:bg-orange-500 hover:text-white">
                            <Edit2 size={16} />
                          </IconButton>

                          <IconButton onClick={() => handleDelete(item.id_skkni)} title="Hapus Data" className="bg-red-50 text-red-600 hover:bg-red-600 hover:text-white">
                            <Trash2 size={16} />
                          </IconButton>
                        </div>
                      </td>
                  ...
                  <tr>
                    <td colSpan="6" className="py-16 text-center">
                      ...
                      <p className="text-sm font-black text-[#071E3D]">Belum ada data SKKNI tersedia.</p>
                    </td>
                  </tr>
                )}
              </tbody>
            ...
            <div className="flex max-h-[92vh] w-full max-w-5xl flex-col overflow-hidden rounded-[34px] border border-slate-100 bg-white shadow-2xl">
              <div className="flex items-start justify-between border-b border-slate-100 p-6">
                <div>
                  ...
                  <p className="mt-1 text-sm font-medium text-slate-400">Lengkapi data standar dan unggah dokumen PDF pendukung.</p>
                </div>
                    ...
                    <FormSection title="Informasi Standar">
                      ...
                      <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                        <div>
                          ...
                          <input type="text" name="legalitas" value={formData.legalitas} onChange={handleInputChange} className={inputClass} placeholder="Contoh: 2026" />
                        </div>
                      ...
                      <div className="rounded-[24px] border border-orange-100 bg-orange-50/60 p-5">
                        ...
                        {isEditing && !dokumenFile && selectedItem?.dokumen && (
                          <p className="mt-3 flex w-fit items-center gap-2 rounded-xl border border-blue-100 bg-blue-50 px-3 py-2 text-xs font-black text-[#071E3D]">
                            ...
                            File saat ini:
                            <span>{selectedItem.dokumen}</span>
                          </p>
                        )}
                      </div>
                    </FormSection>
              ...
              <div className="flex justify-end gap-3 border-t border-slate-100 bg-slate-50/70 p-6">
                ...
                <button type="submit" form="skkniForm" disabled={loading} className="inline-flex items-center gap-2 rounded-2xl bg-orange-500 px-6 py-3 text-xs font-black uppercase tracking-widest text-white shadow-lg shadow-orange-500/20 transition-all hover:bg-[#071E3D] disabled:cursor-not-allowed disabled:bg-slate-300">
            ...
            <div className="flex max-h-[92vh] w-full max-w-4xl flex-col overflow-hidden rounded-[34px] border border-slate-100 bg-white shadow-2xl">
              <div className="flex items-start justify-between border-b border-slate-100 p-6">
                <div>
                  ...
                  <p className="mt-1 text-sm font-medium text-slate-400">Rincian informasi standar dan dokumen pendukung.</p>
                </div>

                <button type="button" onClick={closeDetailModal} className="flex h-10 w-10 items-center justify-center rounded-2xl bg-slate-50 text-slate-400 transition-all hover:bg-red-50 hover:text-red-500">
                  <X size={20} />
                </button>
              </div>
              ...
              <div className="flex-1 overflow-y-auto p-6 custom-scrollbar">
                <div className="space-y-6">
                  <section className="rounded-[28px] border border-slate-100 bg-white p-5 shadow-sm">
                    <h4 className="mb-4 border-b border-slate-100 pb-3 text-sm font-black uppercase tracking-widest text-[#071E3D]">Informasi Standar</h4>

                    <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                      <DetailItem label="Judul Standar" value={selectedItem.judul_skkni} />
                      <DetailItem label="Nomor Standar">
                        <span className="inline-flex rounded-xl bg-orange-50 px-3 py-1.5 font-mono text-orange-500">{selectedItem.no_skkni}</span>
                      </DetailItem>
                  ...
                  <section className="rounded-[28px] border border-slate-100 bg-white p-5 shadow-sm">
                    ...
                    {selectedItem.dokumen ? (
                      <div className="group relative mt-2 h-[500px] overflow-hidden rounded-2xl border border-slate-100 bg-slate-50 shadow-inner">
                        <iframe src={`http://localhost:3000/uploads/${selectedItem.dokumen}#toolbar=0`} className="h-full w-full rounded-2xl" title="Detail PDF" />
              ...
              <div className="flex justify-end border-t border-slate-100 bg-slate-50/70 p-6">
                <button type="button" onClick={closeDetailModal} className="rounded-2xl border border-slate-100 bg-white px-6 py-3 text-xs font-black uppercase tracking-widest text-[#071E3D] transition-all hover:bg-[#071E3D] hover:text-white">
                  Tutup
                </button>
              </div>
            </div>
