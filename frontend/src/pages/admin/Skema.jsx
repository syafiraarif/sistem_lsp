...
import {
  Search, Plus, Edit2, Trash2, X, Save, Loader2, FileText, Upload, BookOpen, Eye, ArrowRight, Filter, Sparkles, Layers, BadgeCheck, FileSearch, ClipboardList, DollarSign
} from "lucide-react";
...
const Skema = () => {
  ...
  const handleFileChange = (e) => {
    ...
    if (file.type !== "application/pdf" && !/\.pdf$/i.test(file.name)) {
      notifikasi.peringatan("Dokumen skema harus menggunakan format PDF.");
      e.target.value = "";
      return;
    }
  ...
  const handleDelete = async (id) => {
    const confirmed = await notifikasi.konfirmasi("Data skema yang dihapus tidak dapat dikembalikan.");
    ...
    try {
      ...
      await notifikasi.sukses("Skema telah dihapus");
      await fetchData();
    } catch (error) {
      notifikasi.gagal(error.response?.data?.message || "Gagal menghapus data");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    if (!isValid) {
      notifikasi.peringatan("Silakan perbaiki isian yang masih kosong/kurang tepat!");
      return;
    }
    ...
    try {
      if (isEditMode) {
        ...
        await notifikasi.sukses("Data skema berhasil diperbarui");
      } else {
        await api.post("/admin/skema", dataToSend, config);
        await notifikasi.sukses("Skema baru berhasil ditambahkan");
      }
      ...
      notifikasi.gagal(error.response?.data?.message || "Terjadi kesalahan saat menyimpan");
    } finally {
      setLoading(false);
    }
  };

  const inputClass = (name) => `w-full rounded-2xl border px-4 py-3 text-sm font-semibold text-[#071E3D] outline-none transition-all disabled:cursor-not-allowed disabled:opacity-60 placeholder:text-slate-300 ${errors[name] ? "border-red-300 bg-red-50 focus:border-red-500 focus:ring-4 focus:ring-red-500/10" : "border-slate-100 bg-slate-50 focus:border-orange-200 focus:bg-white focus:ring-4 focus:ring-orange-500/10"}`;

  const filteredData = data.filter((item) => {
    const search = searchTerm.toLowerCase();
    const matchSearch = (item.judul_skema && item.judul_skema.toLowerCase().includes(search)) || (item.kode_skema && item.kode_skema.toLowerCase().includes(search));
          ...
          <div className="relative z-10 grid grid-cols-1 gap-6 p-6 lg:p-8 xl:grid-cols-[1.15fr_0.85fr]">
            <div className="flex flex-col justify-center">
              <div className="mb-5 inline-flex w-fit items-center gap-2 rounded-full border border-orange-100 bg-orange-50 px-4 py-2">
                ...
                <span className="text-[10px] font-black uppercase tracking-widest text-orange-500">Data Skema Sertifikasi</span>
              </div>
            ...
            <div className="relative overflow-hidden rounded-[32px] bg-[#071E3D] p-6 text-white shadow-2xl shadow-[#071E3D]/15">
              ...
              <div className="relative z-10">
                ...
                <p className="mb-2 text-[10px] font-black uppercase tracking-widest text-white/50">Total Skema</p>
                <h2 className="mb-4 text-5xl font-black leading-none">{data.length}</h2>
                <p className="text-sm font-medium leading-relaxed text-white/60">Ringkasan status skema yang tersedia dalam sistem.</p>
        ...
        <section className="overflow-hidden rounded-[32px] border border-slate-100 bg-white shadow-sm">
          <div className="border-b border-slate-100 p-6">
            <div className="mb-4 inline-flex items-center gap-2 rounded-full border border-orange-100 bg-orange-50 px-4 py-2">
              ...
              <span className="text-[10px] font-black uppercase tracking-widest text-orange-500">Daftar Skema</span>
            </div>

            <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
              <div>
                <h2 className="text-2xl font-black text-[#071E3D]">Data Skema Sertifikasi</h2>
                <p className="mt-2 text-sm font-medium text-slate-400">Cari berdasarkan kode atau judul, lalu filter berdasarkan status.</p>
              </div>
                ...
                {loading ? (
                  <tr>
                    <td colSpan="6" className="py-16 text-center">
                      ...
                      <p className="text-sm font-black uppercase tracking-widest text-[#071E3D]">Memuat Data Skema</p>
                    </td>
                  </tr>
                  ...
                  <tr>
                    <td colSpan="6" className="py-16 text-center">
                      ...
                      <p className="text-sm font-black text-[#071E3D]">Belum ada data skema ditemukan.</p>
                    </td>
                  </tr>
                ) : (
                  filteredData.map((item, index) => (
                    <tr key={item.id_skema} className="border-b border-slate-100 transition-all last:border-0 hover:bg-orange-50/30">
                      ...
                      <td className="px-5 py-4">
                        <span className="font-mono text-sm font-black text-orange-500">{item.kode_skema}</span>
                      </td>
                      ...
                      <td className="px-5 py-4 text-center">
                        <div className="flex flex-col items-center justify-center gap-2">
                          <button type="button" onClick={() => navigate(`/admin/skema/${item.id_skema}/persyaratan`)} className="w-[155px] rounded-xl border border-slate-100 bg-slate-50 px-3 py-2 text-[11px] font-black text-[#071E3D] transition-all hover:border-[#071E3D] hover:bg-[#071E3D] hover:text-white">
                            Persyaratan Dasar
                          </button>
                          <button type="button" onClick={() => navigate(`/admin/skema/${item.id_skema}/persyaratan-tuk`)} className="w-[155px] rounded-xl border border-orange-100 bg-orange-50 px-3 py-2 text-[11px] font-black text-orange-500 transition-all hover:bg-orange-500 hover:text-white">
                            Persyaratan TUK
                          </button>
                        </div>
                      </td>
                      ...
                      <td className="px-5 py-4 text-center">
                        <div className="flex items-center justify-center gap-2">
                          <IconButton onClick={() => navigate(`/admin/skema/${item.id_skema}/biaya-uji`)} title="Atur Biaya Uji" className="bg-emerald-50 text-emerald-600 hover:bg-emerald-600 hover:text-white">
                            <DollarSign size={16} />
                          </IconButton>

                          <IconButton onClick={() => handleDetail(item)} title="Detail Skema" className="bg-blue-50 text-blue-600 hover:bg-blue-600 hover:text-white">
                            <Eye size={16} />
                          </IconButton>

                          <IconButton onClick={() => handleEdit(item)} title="Edit" className="bg-orange-50 text-orange-500 hover:bg-orange-500 hover:text-white">
                            <Edit2 size={16} />
                          </IconButton>

                          <IconButton onClick={() => handleDelete(item.id_skema)} title="Hapus" className="bg-red-50 text-red-600 hover:bg-red-600 hover:text-white">
                            <Trash2 size={16} />
                          </IconButton>
                        </div>
                      </td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#071E3D]/60 p-4 backdrop-blur-sm">
          <div className="flex max-h-[92vh] w-full max-w-5xl flex-col overflow-hidden rounded-[34px] border border-slate-100 bg-white shadow-2xl">
            <div className="flex items-start justify-between border-b border-slate-100 p-6">
              ...
              <button type="button" onClick={() => setShowDetailModal(false)} className="flex h-10 w-10 items-center justify-center rounded-2xl bg-slate-50 text-slate-400 transition-all hover:bg-red-50 hover:text-red-500">
                <X size={20} />
              </button>
            </div>
              ...
              <div className="space-y-6">
                <InfoPanel title="Informasi Utama">
                  <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                    ...
                    <DetailItem label="Judul Skema" wide>
                      ...
                      {selectedSkema.judul_skema_en && <p className="mt-1 text-sm font-semibold italic text-slate-400">{selectedSkema.judul_skema_en}</p>}
                    </DetailItem>
                  </div>
                </InfoPanel>
                  ...
                  <InfoPanel title="Preview Dokumen Skema">
                    <div className="flex min-h-[380px] flex-col overflow-hidden rounded-2xl border border-slate-100">
                      <div className="flex items-center justify-between border-b border-slate-100 bg-slate-50 px-4 py-3">
                        <span className="text-xs font-black uppercase tracking-widest text-[#071E3D]">Dokumen Skema</span>
                ...
                <InfoPanel title="Navigasi Instrumen & Asesmen">
                  <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
                    <NavigationCard title="FR.IA.01" subtitle="Observasi" onClick={() => navigate(`/admin/skema/${selectedSkema.id_skema}/ia01`)} className="border-blue-100 bg-blue-50 text-blue-700 hover:bg-blue-600 hover:text-white" />
                    <NavigationCard title="FR.IA.03" subtitle="Pertanyaan" onClick={() => navigate(`/admin/skema/${selectedSkema.id_skema}/ia03`)} className="border-indigo-100 bg-indigo-50 text-indigo-700 hover:bg-indigo-600 hover:text-white" />
                    <NavigationCard title="FR.MAPA" subtitle="Manajemen" onClick={() => navigate(`/admin/skema/${selectedSkema.id_skema}/mapa`)} className="border-orange-100 bg-orange-50 text-orange-500 hover:bg-orange-500 hover:text-white" />
                    <NavigationCard title="Kelompok" subtitle="Pekerjaan" onClick={() => navigate(`/admin/skema/${selectedSkema.id_skema}/kelompok-pekerjaan`)} className="border-amber-100 bg-amber-50 text-amber-700 hover:bg-amber-600 hover:text-white" />
                  </div>
                </InfoPanel>
              </div>
            </div>
          ...
          <div className="flex max-h-[92vh] w-full max-w-5xl flex-col overflow-hidden rounded-[34px] border border-slate-100 bg-white shadow-2xl">
            <div className="flex items-start justify-between border-b border-slate-100 p-6">
              <div>
                ...
                <p className="mt-1 text-sm font-medium text-slate-400">Lengkapi informasi utama, atribut skema, dan dokumen pendukung.</p>
              </div>

              <button type="button" onClick={() => setShowModal(false)} className="flex h-10 w-10 items-center justify-center rounded-2xl bg-slate-50 text-slate-400 transition-all hover:bg-red-50 hover:text-red-500">
                <X size={20} />
              </button>
            </div>
            ...
            <div className="flex justify-end gap-3 border-t border-slate-100 bg-slate-50/70 p-6">
              <button type="button" className="rounded-2xl border border-slate-100 bg-white px-6 py-3 text-xs font-black uppercase tracking-widest text-[#071E3D] transition-all hover:bg-[#071E3D] hover:text-white" onClick={() => setShowModal(false)}>
                Batal
              </button>

              <button type="submit" form="skemaForm" disabled={loading} className="inline-flex items-center gap-2 rounded-2xl bg-orange-500 px-6 py-3 text-xs font-black uppercase tracking-widest text-white shadow-lg shadow-orange-500/20 transition-all hover:bg-[#071E3D] disabled:cursor-not-allowed disabled:bg-slate-300">
...
function MiniStat({ icon, label, value }) {
  return (
    <div className="flex items-center gap-4 rounded-[28px] border border-slate-100 bg-white p-5 shadow-sm">
      <div className="flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl bg-orange-50 text-orange-500">{icon}</div>
      <div>
...
function StatusBadge({ status }) {
  const style = status === "aktif" ? "bg-green-50 text-green-700 border-green-200" : status === "nonaktif" ? "bg-red-50 text-red-700 border-red-200" : "bg-slate-100 text-slate-600 border-slate-200";

  return <span className={`inline-flex rounded-full border px-3 py-1 text-[10px] font-black uppercase tracking-widest ${style}`}>{status || "draft"}</span>;
}

function IconButton({ children, onClick, title, className }) {
  return (
    <button type="button" onClick={onClick} title={title} className={`inline-flex h-10 w-10 items-center justify-center rounded-xl transition-all ${className}`}>
      {children}
    </button>
  );
}
...
function InfoPanel({ title, children }) {
  return (
    <section className="rounded-[28px] border border-slate-100 bg-white p-5 shadow-sm">
      <h4 className="mb-4 border-b border-slate-100 pb-3 text-sm font-black uppercase tracking-widest text-[#071E3D]">{title}</h4>
      {children}
    </section>
  );
}
...
function NavigationCard({ title, subtitle, onClick, className }) {
  return (
    <button type="button" onClick={onClick} className={`group flex min-h-[112px] flex-col items-center justify-center rounded-3xl border p-4 text-center shadow-sm transition-all ${className}`}>
...
function FormSection({ title, children }) {
  return (
    <section className="rounded-[28px] border border-slate-100 bg-white p-5 shadow-sm">
      <h4 className="mb-5 border-b border-slate-100 pb-4 text-sm font-black uppercase tracking-widest text-[#071E3D]">{title}</h4>
...
function Label({ children }) {
  return <label className="mb-1.5 block text-[10px] font-black uppercase tracking-widest text-slate-400">{children}</label>;
}

function ErrorText({ children }) {
  return <span className="mt-1 block text-xs font-bold text-red-500">{children}</span>;
}
