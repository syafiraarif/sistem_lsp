...
import {
  Database, Plus, Edit2, Trash2, X, Save, Loader2, Filter, Settings, FileText, List, ArrowLeft, Sparkles, ClipboardList, BadgeCheck, Layers
} from "lucide-react";
...
const BankSoal = () => {
  ...
  useEffect(() => {
    if (unitQuery) setFilterUnit(unitQuery);
  }, [unitQuery]);
  ...
  const fetchSoal = async () => {
    ...
    try {
      ...
      notifikasi.gagal("Gagal mengambil data soal");
    } finally {
      setLoading(false);
    }
  };
  ...
  const validateForm = () => {
    if (!formData.id_unit) return "Silakan pilih Unit Kompetensi terlebih dahulu!";
    const tanya = String(formData.pertanyaan).trim();
    if (!tanya || tanya.length < 4) return "Teks pertanyaan terlalu pendek. Minimal harus 4 karakter!";
    return null;
  };
  ...
  const handleSave = async (e) => {
    ...
    if (errorMsg) {
      notifikasi.peringatan(errorMsg);
      return;
    }
    ...
    try {
      if (editingId) {
        ...
        await notifikasi.sukses("Soal berhasil diperbarui");
      } else {
        await api.post("/admin/bank-soal", formData);
        await notifikasi.sukses("Soal berhasil ditambahkan");
      }

      closeModal();
      await fetchSoal();
    } catch (error) {
      notifikasi.gagal(error.response?.data?.message || "Gagal menyimpan soal");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (id) => {
    const confirmed = await notifikasi.konfirmasi("Yakin ingin menghapus data Soal ini? Aksi ini tidak dapat dibatalkan!");

    if (!confirmed) return;

    try {
      await api.delete(`/admin/bank-soal/${id}`);
      await notifikasi.sukses("Soal berhasil dihapus");
      fetchSoal();
    } catch (error) {
      notifikasi.gagal(error.response?.data?.message || "Gagal menghapus soal");
    }
  };
    ...
    <div className="min-h-screen bg-[#F8FAFC] p-4 md:p-6 lg:p-8">
      <div className="mx-auto max-w-7xl space-y-6">
          ...
          <div className="relative z-10 grid grid-cols-1 gap-6 p-6 lg:p-8 xl:grid-cols-[1.15fr_0.85fr]">
            <div className="flex flex-col justify-center">
              <div className="mb-5 inline-flex w-fit items-center gap-2 rounded-full border border-orange-100 bg-orange-50 px-4 py-2">
                ...
                <span className="text-[10px] font-black uppercase tracking-widest text-orange-500">Bank Soal</span>
              </div>
              ...
              <p className="mt-5 max-w-2xl text-base font-medium leading-relaxed text-slate-500 lg:text-lg">
                {unitQuery && activeUnitInfo ? `Mengelola bank soal khusus untuk unit: ${activeUnitInfo.kode_unit}` : "Pengelolaan bank soal untuk ujian tertulis, esai, lisan, dan wawancara asesi."}
              </p>

              <div className="mt-7 flex flex-col gap-3 sm:flex-row">
                {unitQuery && (
                  <button type="button" onClick={() => navigate("/admin/unit-kompetensi")} className="inline-flex items-center justify-center gap-2 rounded-2xl border border-slate-100 bg-slate-50 px-7 py-4 text-xs font-black uppercase tracking-widest text-[#071E3D] transition-all hover:bg-[#071E3D] hover:text-white">
                    <ArrowLeft size={17} />
                    Kembali
                  </button>
                )}

                <button type="button" onClick={openModal} className="inline-flex items-center justify-center gap-2 rounded-2xl bg-orange-500 px-7 py-4 text-xs font-black uppercase tracking-widest text-white shadow-lg shadow-orange-500/20 transition-all hover:bg-[#071E3D]">
                  <Plus size={17} />
                  Tambah Soal
                </button>
              </div>
            ...
            <div className="relative overflow-hidden rounded-[32px] bg-[#071E3D] p-6 text-white shadow-2xl shadow-[#071E3D]/15">
              ...
              <div className="relative z-10">
                ...
                <p className="mb-2 text-[10px] font-black uppercase tracking-widest text-white/50">Ringkasan Soal</p>
                <h2 className="mb-4 text-2xl font-black">{soalList.length} Soal Tersedia</h2>

                <p className="text-sm font-medium leading-relaxed text-white/60">
                  Filter soal berdasarkan unit kompetensi dan jenis instrumen asesmen.
                </p>
        ...
        <section className="grid grid-cols-1 gap-5 md:grid-cols-3">
          <MiniStat icon={<ClipboardList size={22} />} label="Total Soal" value={`${soalList.length} Soal`} />
          <MiniStat icon={<BadgeCheck size={22} />} label="Soal Aktif" value={`${totalAktif} Aktif`} tone="green" />
          <MiniStat icon={<Layers size={22} />} label="Hasil Filter" value={`${filteredSoal.length} Data`} tone="navy" />
        </section>
          ...
          <div className="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
            <div>
              <div className="mb-4 inline-flex items-center gap-2 rounded-full border border-orange-100 bg-orange-50 px-4 py-2">
                ...
                <span className="text-[10px] font-black uppercase tracking-widest text-orange-500">Filter Soal</span>
              </div>

              <h2 className="text-2xl font-black text-[#071E3D]">Daftar Bank Soal</h2>
              <p className="mt-2 text-sm font-medium text-slate-400">Pilih unit kompetensi dan jenis soal untuk memfilter data.</p>
            </div>

            <div className="flex flex-col gap-3 md:flex-row">
              <div className="relative w-full md:w-64">
                <Filter size={18} className={`absolute left-4 top-1/2 -translate-y-1/2 ${unitQuery ? "text-slate-300" : "text-orange-500"}`} />
                <select
                  className={`w-full appearance-none rounded-2xl border px-12 py-4 text-sm font-black outline-none transition-all ${unitQuery ? "cursor-not-allowed border-slate-100 bg-slate-100 text-slate-400" : "border-slate-100 bg-slate-50 text-[#071E3D] focus:border-orange-200 focus:bg-white focus:ring-4 focus:ring-orange-500/10"}`}
                  value={filterUnit}
                  disabled={!!unitQuery}
              ...
              <div className="relative w-full md:w-64">
                <Settings size={18} className="absolute left-4 top-1/2 -translate-y-1/2 text-orange-500" />
                <select
              ...
              <button type="button" onClick={openModal} className="inline-flex items-center justify-center gap-2 rounded-2xl bg-orange-500 px-6 py-4 text-xs font-black uppercase tracking-widest text-white shadow-lg shadow-orange-500/20 transition-all hover:bg-[#071E3D]">
                <Plus size={16} />
                Tambah Soal
              </button>
            </div>
                ...
                {loading ? (
                  <tr>
                    <td colSpan="6" className="p-16 text-center">
                      <Loader2 className="mx-auto mb-4 animate-spin text-orange-500" size={38} />
                      <p className="font-black text-[#071E3D]">Memuat data...</p>
                    </td>
                  </tr>
                ) : filteredSoal.length > 0 ? (
                  filteredSoal.map((item, idx) => (
                    <tr key={item.id_soal} className="border-b border-slate-100 text-sm transition-all last:border-0 hover:bg-orange-50/30">
                      <td className="px-5 py-4 text-center font-bold text-slate-500">{idx + 1}</td>
                      ...
                      <td className="px-5 py-4">
                        <span className="inline-flex rounded-full bg-slate-50 px-4 py-2 text-[10px] font-black uppercase tracking-widest text-[#071E3D]">
                          {item.jenis === "IA05_pg" ? "PILIHAN GANDA" : item.jenis === "IA06_essay" ? "ESAI" : item.jenis === "IA07_lisan" ? "LISAN" : item.jenis === "IA09_wawancara" ? "WAWANCARA" : item.jenis}
                        </span>
                      </td>

                      <td className="px-5 py-4 text-slate-500">
                        <div className="line-clamp-2 max-w-md font-semibold" title={item.pertanyaan}>{item.pertanyaan}</div>
                      </td>
                      ...
                      <td className="px-5 py-4 text-center">
                        <div className="flex items-center justify-center gap-2">
                          ...
                          <button type="button" onClick={() => handleEdit(item)} className="rounded-xl bg-orange-50 p-2 text-orange-500 transition-all hover:bg-orange-500 hover:text-white" title="Edit Soal">
                            <Edit2 size={16} />
                          </button>

                          <button type="button" onClick={() => handleDelete(item.id_soal)} className="rounded-xl border border-red-100 bg-red-50 p-2 text-red-500 transition-all hover:bg-red-500 hover:text-white" title="Hapus Soal">
                            <Trash2 size={16} />
                          </button>
                        </div>
                      </td>
                  ...
                  <tr>
                    <td colSpan="6" className="p-16 text-center">
                      ...
                      <p className="font-black text-[#071E3D]">Belum ada data soal ditemukan.</p>
                    </td>
                  </tr>
                )}
              </tbody>
          ...
          <div className="flex w-full max-w-2xl flex-col overflow-hidden rounded-[34px] border border-slate-100 bg-white shadow-2xl">
            <div className="flex items-center justify-between border-b border-slate-100 p-6">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h3 className="text-xl font-black text-[#071E3D]">{editingId ? "Edit Bank Soal" : "Tambah Bank Soal Baru"}</h3>
                  <p className="mt-1 text-sm font-medium text-slate-400">Lengkapi unit kompetensi, jenis, tingkat kesulitan, dan isi pertanyaan.</p>
                </div>
              </div>
              ...
              <form id="soalForm" onSubmit={handleSave} className="space-y-5">
                <div>
                  <label className="mb-2 block text-[10px] font-black uppercase tracking-widest text-slate-400">
                    ...
                    {unitQuery && <span className="font-bold normal-case tracking-normal text-red-500">(Terkunci)</span>}
                  </label>
                  ...
                  <select
                    ...
                    disabled={!!unitQuery}
                    className={`w-full rounded-2xl border px-4 py-4 text-sm font-black outline-none transition-all ${unitQuery ? "cursor-not-allowed border-slate-100 bg-slate-100 text-slate-400" : "border-slate-100 bg-slate-50 text-[#071E3D] focus:border-orange-200 focus:bg-white focus:ring-4 focus:ring-orange-500/10"}`}
                  >
                ...
                <div className="grid grid-cols-1 gap-5 md:grid-cols-2">
                  <div>
                    <label className="mb-2 block text-[10px] font-black uppercase tracking-widest text-slate-400">Jenis Soal</label>
                    <select
                      name="jenis"
                  ...
                  <div>
                    <label className="mb-2 block text-[10px] font-black uppercase tracking-widest text-slate-400">Tingkat Kesulitan</label>
                    <select
                ...
                <div>
                  <label className="mb-2 block text-[10px] font-black uppercase tracking-widest text-slate-400">Isi Pertanyaan</label>
                  <textarea
                    name="pertanyaan"
                    rows="5"
