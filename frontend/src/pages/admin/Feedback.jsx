...
import { 
  ...
  MessageSquare, Star, Eye, EyeOff, FileText
} from 'lucide-react';
...
const FeedbackAdmin = () => {
  ...
  const handleToggleStatus = async (id, currentStatus) => {
    try {
      ...
      Swal.fire({ toast: true, position: 'top-end', icon: 'success', title: `Status berhasil diubah!`, showConfirmButton: false, timer: 1500 });
      fetchFeedbacks(); 
    } catch (err) {
  ...
  const renderStars = (rating) => {
    const stars = [];
    for (let i = 1; i <= 5; i++) {
      stars.push(<Star key={i} size={14} className={`${i <= rating ? 'fill-yellow-400 text-yellow-400' : 'text-gray-300 fill-transparent'}`} />);
    }
  ...
  return (
    <div className="p-6 md:p-8 bg-[#FAFAFA] min-h-screen flex flex-col gap-6">
      ...
      <div className="bg-white border border-[#071E3D]/10 rounded-xl shadow-sm p-6">
        ...
        {/* Search Bar & Title - Icon diubah menjadi FileText */}
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
          <h4 className="text-[16px] font-bold text-[#071E3D] m-0 flex items-center gap-2">
            <FileText size={18} className="text-[#CC6B27]"/> Daftar Ulasan Masuk
          </h4>
          <div className="w-full md:w-80 relative group">
            ...
            <input 
              type="text" placeholder="Cari nama, peran, atau isi ulasan..." 
                ...
                currentData.map((item, index) => (
                  <tr key={item.id_feedback} className="border-b border-[#071E3D]/5 hover:bg-[#CC6B27]/5 transition-colors">
                    ...
                    <td className="py-3 px-4">
                      ...
                      <div className="text-[11px] text-[#CC6B27] mt-0.5 capitalize font-medium">{item.peran?.replace('_', ' ')}</div>
                    </td>
                    <td className="py-3 px-4 text-[#182D4A] text-[13px] max-w-xs truncate" title={item.pesan}>{item.pesan}</td>
                    <td className="py-3 px-4 text-center">{renderStars(item.rating)}</td>
                    <td className="py-3 px-4 text-center">
                      <span className={`px-3 py-1 inline-flex text-[11px] leading-5 font-black uppercase tracking-wider rounded-full border ${item.status === 'aktif' ? 'bg-emerald-50 text-emerald-600 border-emerald-200' : 'bg-slate-100 text-slate-500 border-slate-200'}`}>
                        {item.status === 'aktif' ? 'Ditampilkan' : 'Disembunyikan'}
                      </span>
                    </td>
                    <td className="py-3 px-4 text-center">
                      <div className="flex justify-center gap-2">
                        <button onClick={() => handleToggleStatus(item.id_feedback, item.status)} className={`p-1.5 rounded-lg transition-colors ${item.status === 'aktif' ? 'text-slate-500 bg-slate-100 hover:bg-slate-200 hover:text-slate-700' : 'text-emerald-600 bg-emerald-50 hover:bg-emerald-100 hover:text-emerald-700'}`} title={item.status === 'aktif' ? "Sembunyikan dari Publik" : "Tampilkan ke Publik"}>
                          {item.status === 'aktif' ? <EyeOff size={16} /> : <Eye size={16} />}
                        </button>
                        <button onClick={() => handleDelete(item.id_feedback)} className="p-1.5 text-red-600 bg-red-50 rounded-lg hover:bg-red-600 hover:text-white border border-red-100 transition-colors" title="Hapus"><Trash2 size={16} /></button>
                      </div>
                    </td>
                  </tr>
                ))
