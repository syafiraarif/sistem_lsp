...
const ValidatorMKVA = () => {
  ...
  const normalizeActiveAsesor = (item) => {
    ...
    return {
      ...
      no_hp:
        user?.no_hp || item?.no_hp || profile?.no_hp || "-",
    };
  };
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      console.error("Error fetch validator MKVA:", err);
      ...
      alert(err?.response?.data?.message || "Gagal memuat data validator");
    } finally {
      setLoading(false);
    }
  ...
  const handleSave = async () => {
    if (selected.length === 0) {
      alert("Pilih minimal 1 validator dulu");
      return;
    }
    ...
    if (mergedIds.length < 2) {
      alert("Validator MKVA minimal 2 asesor");
      return;
    }

    if (mergedIds.length > 3) {
      alert("Validator MKVA maksimal 3 asesor");
      return;
    }
    ...
    try {
      ...
      setSelected([]);
      await fetchData();
      alert("Validator MKVA berhasil disimpan");
    } catch (err) {
      console.error("Error save validator MKVA:", err);
      alert(err?.response?.data?.message || "Gagal menyimpan validator");
    } finally {
      setSaving(false);
    }
  };

  const handleDeleteAsesor = async (id_user, nama = "validator ini") => {
    if (!window.confirm(`Hapus ${nama} dari validator MKVA?`)) return;
    ...
    try {
      ...
      await fetchData();
      alert("Validator berhasil dihapus");
    } catch (err) {
      console.error("Error delete validator MKVA:", err);
      alert(err?.response?.data?.message || "Gagal menghapus validator");
    }
  };
              ...
              >
                {activeAsesor.length === 0 ? (
                  <EmptyState
                    ...
                    desc="Tambahkan minimal 2 asesor validator MKVA."
                  />
                ) : (
                  ...
                  <EmptyState
                    ...
                    title={
                      search
                        ...
                        : "Semua asesor sudah menjadi validator"
                    }
                    desc={
                      search
                    ...
                    <div className="relative z-10 flex flex-col md:flex-row md:items-center md:justify-between gap-5">
                      <div>
                        ...
                        <p className="text-white/50 text-sm font-bold mt-1">
                          ...
                          {activeAsesor.length + selected.length} validator
                        </p>
                      </div>
