// frontend/src/pages/Asesor/FRIA03Asesor.jsx
...
export default function FRIA03Asesor() {
  ...
  const fetchData = async () => {
    try {
      ...
      const res = await api.get(`/asesor/fr-ia03/asesor/${idFrIa03}`);
      ...
      Swal.fire(
        "Gagal",
        err.response?.data?.message || "Gagal memuat FR.IA.03 Asesor Penguji",
        "error"
      );
    } finally {
      setLoading(false);
  ...
  const downloadPdf = () => {
    ...
    window.open(
      `${api.defaults.baseURL}/asesor/fr-ia03/asesor/${idFrIa03}/pdf`,
      "_blank"
    );
  };
  ...
  if (loading) {
    return <LoadingScreen title="Memuat FR.IA.03 Asesor Penguji..." />;
  }
    ...
    <div className="min-h-screen bg-slate-100 py-6 print:bg-white print:py-0">
      ...
      <main className="mx-auto w-[900px] bg-white px-10 py-8 text-[14px] text-black shadow-lg print:w-full print:shadow-none print:px-8 print:py-6">
        ...
        {Object.keys(kelompokMap).length === 0 ? (
          <EmptyState text="Belum ada pertanyaan FR.IA.03 dari Komite Teknis." />
        ) : (
