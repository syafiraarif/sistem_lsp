// frontend/src/pages/Asesor/DetailPesertaAsesor.jsx
...
export default function DetailPesertaAsesor() {
  ...
  const fetchPeserta = async () => {
    try {
      ...
      setPeserta(data);
      setHasilKeputusan(
        data?.hasil_keputusan?.hasil ||
        data?.status_asesmen ||
        ""
      );
      setNilaiAkhir(
        data?.hasil_keputusan?.nilai_akhir ??
        data?.nilai_akhir ??
        ""
      );
      setCatatanAsesor(
        data?.hasil_keputusan?.catatan_asesor ||
        data?.keterangan ||
        ""
      );
    } catch (err) {
  ...
  try {
    ...
    await axios.put(
      `http://localhost:3000/api/asesor/peserta/${id_peserta}/keputusan`,
      {
    ...
    Swal.fire({
      icon: "success",
      title: "Berhasil",
      text: "Hasil asesmen berhasil disimpan.",
      confirmButtonColor: "#CC6B27",
    });
  } catch (err) {
  ...
  <div className="grid grid-cols-1 gap-5 md:grid-cols-2">
    <div>
      ...
      <select
        ...
        onChange={(e) => setHasilKeputusan(e.target.value)}
    ...
    <div className="md:col-span-2">
      ...
      <textarea
        ...
        rows={5}
        placeholder="Masukkan catatan atau keterangan hasil asesmen peserta..."
