...
export default function JadwalMkva() {
  ...
  const handleDownloadPdf = async (item) => {
    ...
    try {
      ...
      console.error(err);
      setError(err.response?.data?.message || "Gagal download PDF MKVA. Pastikan dokumen MKVA sudah dibuat.");
    } finally {
              ...
              <div className="relative overflow-hidden rounded-[32px] bg-[#071E3D] p-6 text-white shadow-2xl shadow-[#071E3D]/15">
                ...
                <div className="relative z-10 flex h-full flex-col">
                  ...
                  <div className="mt-auto pt-6 grid grid-cols-2 gap-3">
                    ...
                    <HeroPill label="Selesai" value={`${totalSelesai} Dokumen`} />
                  </div>
                </div>
              </div>
            </div>
...
function JadwalMkvaCard({ item, index, downloadingId, onDownload }) {
  ...
  const handleOpenMkva = () => {
    if (!idJadwal || !bolehMkva) return;
    navigate(`/asesor/mkva/jadwal/${idJadwal}`, { state: { item } });
  };
  return (
...
function formatRentangTanggal(start, end) {
  ...
  if (String(start).slice(0, 10) === String(end).slice(0, 10)) return formatTanggal(start);
