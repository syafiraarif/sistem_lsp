...
export default function JadwalAsesi() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      console.error(err);
      setError(err.response?.data?.message || "Gagal memuat jadwal tersedia.");
      setJadwal([]);
      setMyJadwal([]);
    } finally {
      setLoading(false);
            ...
            <div className="relative z-10 grid grid-cols-1 xl:grid-cols-[1.15fr_0.85fr] gap-6 p-6 lg:p-8">
              <div className="flex flex-col justify-center">
                ...
                <p className="mt-5 max-w-2xl text-base lg:text-lg font-medium leading-relaxed text-slate-500">
                  Temukan jadwal uji kompetensi yang tersedia, pilih jadwal
                  sesuai skema, lalu lanjutkan proses pendaftaran asesmen.
                </p>
              ...
              <div className="relative overflow-hidden rounded-[32px] bg-[#071E3D] p-6 text-white shadow-2xl shadow-[#071E3D]/15">
                ...
                <div className="relative z-10">
                  ...
                  <p className="text-sm font-medium leading-relaxed text-white/60">
                    Pilih jadwal terlebih dahulu agar dapat lanjut ke proses
                    pembayaran dan asesmen.
                  </p>
                  ...
                  <div className="mt-6 grid grid-cols-2 gap-3">
                    ...
                    <HeroPill
                      label="Belum"
                      value={`${totalBelumDipilih} Jadwal`}
                    />
                  </div>
                </div>
              </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-[1fr_330px]">
        ...
        <div className="border-t xl:border-t-0 xl:border-l border-slate-100 bg-slate-50/60 p-6 flex flex-col justify-between gap-4">
          <div>
            ...
            <p className="mt-2 text-sm font-medium leading-relaxed text-slate-500">
              Setelah memilih jadwal, Anda dapat lanjut melihat status di menu
              Jadwal Saya.
            </p>
          </div>
          ...
          {!sudahDipilih ? (
            ...
            <div className="grid grid-cols-1 gap-3">
              <button
                disabled
                className="flex w-full items-center justify-center gap-2 rounded-2xl border border-emerald-100 bg-emerald-50 px-5 py-4 text-xs font-black uppercase tracking-widest text-emerald-600 cursor-not-allowed"
              >
                <CheckCircle size={16} />
                Sudah Dipilih
              </button>
    ...
    <div className="min-h-screen bg-[#F8FAFC] flex items-center justify-center px-5">
      <div className="bg-white rounded-[32px] border border-slate-100 shadow-xl p-10 text-center max-w-sm w-full">
        ...
        <p className="text-slate-500 text-sm mt-2 font-medium">
          Mengambil data jadwal sertifikasi yang tersedia.
        </p>
      </div>
    </div>
  );
